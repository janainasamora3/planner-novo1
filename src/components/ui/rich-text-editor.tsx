...
const FONT_SIZES = [
  ...
  { label: "Média", value: "18px" },
  { label: "Grande", value: "22px" },
  { label: "XG", value: "28px" },
  { label: "XXG", value: "36px" },
];
...
// =================== TextStyle augmentation: fontSize + highlight ===================
// The proper Tiptap v3 pattern is `addGlobalAttributes()` registered on a
// top-level Extension — this makes the new attributes part of the `textStyle`
// mark so `setMark("textStyle", { fontSize })` actually applies them.
...
function Toolbar({ editor }: { editor: Editor }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-0.5 border-b border-border bg-muted/30 p-1.5">
      ...
      <button type="button" onClick={() => editor.chain().focus().toggleTaskList().run()} className={btnClass(editor.isActive("taskList"))} title="Checkbox inline">
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none"><path d="M9 11l3 3L22 4M21 12v7a2 2 0 01-2 2H5a2 2 0 01-2-2V5a2 2 0 012-2h11" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
      </button>
      ...
      <button type="button" onClick={() => editor.chain().focus().setHorizontalRule().run()} className={btnClass()} title="Linha horizontal">
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none"><path d="M3 12h18" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/></svg>
      </button>
      <Divider />
...
function HighlightPickerButton({ editor }: { editor: Editor }) {
  const [open, setOpen] = useState(false);
  ...
  return (
    <div ref={containerRef} className="relative">
      <button
        ...
        title="Marca-texto"
      >
        <span className="text-base font-bold" style={{ backgroundColor: currentColor || "transparent" }}>H</span>
      </button>
      {open && (
        <div className="absolute top-9 left-0 z-50 p-2 rounded-md border border-border bg-popover shadow-lg">
          <div className="grid grid-cols-4 gap-1">
            {HIGHLIGHT_COLORS.map((c) => (
              <button
                ...
                type="button"
                onClick={() => {
                  (editor.chain().focus() as any).setHighlight(c).run();
                  setOpen(false);
                }}
                className="h-6 w-6 rounded-md border border-border"
                style={{ background: c }}
          ...
          <div className="mt-2 flex items-center gap-1.5">
            <input
              type="color"
              onChange={(e) => (editor.chain().focus() as any).setHighlight(e.target.value).run()}
              className="h-7 w-7 cursor-pointer rounded border-0 bg-transparent p-0"
            />
            <button
              type="button"
...
function EmojiPickerButton({ editor }: { editor: Editor }) {
  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      {open && (
        <div className="absolute top-9 right-0 z-50 w-72 p-2 rounded-md border border-border bg-popover shadow-lg">
          <div className="flex gap-1 mb-2 border-b border-border pb-1.5 overflow-x-auto">
            {EMOJI_CATEGORIES.map((c, i) => (
              <button
                ...
                className={cn(
                  ...
                  category === i ? "bg-primary/15 text-primary" : "text-muted-foreground hover:text-foreground hover:bg-muted"
                )}
              >
                {c.name}
              </button>
          ...
          <div className="grid grid-cols-8 gap-0.5 max-h-48 overflow-y-auto">
            {EMOJI_CATEGORIES[category].emojis.map((e, i) => (
              <button
                ...
                className="h-7 w-7 rounded text-base hover:bg-muted transition-colors flex items-center justify-center"
                title={e}
              >
                {e}
              </button>
