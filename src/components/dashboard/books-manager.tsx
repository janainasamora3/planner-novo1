...
export function BooksManager({ page, onClose }: BooksManagerProps) {
  ...
  const [view, setView] = useState<"acervo" | "agenda">("acervo");
  ...
  function registrarLeitura(bookId: string, paginas: number) {
    const book = books.find((b) => b.id === bookId);
    if (!book) return;
    const today = new Date().toISOString().slice(0, 10);
    const log = { ...(book.readingLog ?? {}) };
    log[today] = (log[today] ?? 0) + paginas;
  ...
  function cycleStatus(id: string, current: BookStatus) {
    ...
    if (next === "lido" && !current.includes("lido")) {
      patch.finishDate = new Date().toISOString().slice(0, 10);
...
function ReadingAgenda({
  ...
  books: Book[];
  onRegisterReading: (bookId: string, paginas: number) => void;
  ...
  function handleRegister(bookId: string) {
    ...
    onRegisterReading(bookId, paginas);
    setPagesInput("");
  }
                ...
                <div key={book.id} className="bg-card border border-border rounded-lg p-3">
                  ...
                  <div className="flex items-center gap-1.5">
                    <input
                      ...
                      className="flex-1 h-8 text-sm bg-muted/30 border border-border rounded px-2 focus:outline-none"
                    />
                    <button onClick={() => handleRegister(book.id)} className="h-8 px-3 rounded bg-blue-600 hover:bg-blue-500 text-white text-xs font-medium">+ Registrar</button>
                    {book.totalPages && (book.pagesRead ?? 0) >= book.totalPages && (
                      <button onClick={() => onUpdateBook(book.id, { status: "lido", finishDate: todayIso })} className="h-8 px-3 rounded bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-medium">✓ Terminei!</button>
                    )}
                  </div>
                </div>
              );
