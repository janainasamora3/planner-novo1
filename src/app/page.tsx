...
import { ExerciciosManager } from "@/components/dashboard/exercicios-manager";
import { CursosManager } from "@/components/dashboard/cursos-manager";
...
export default function Home() {
  ...
  const [exerciciosPage, setExerciciosPage] = useState<PageCard | null>(null);
  const [cursosPage, setCursosPage] = useState<PageCard | null>(null);
  ...
  function openPage(page: PageCard) {
    if (page.special === "social-media") {
      ...
      setFitnessPage(page);
    } else if (page.special === "exercicios") {
      setExerciciosPage(page);
    } else if (page.special === "cursos") {
      setCursosPage(page);
    } else {
      setDetailPage(page);
    }
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      {exerciciosPage && (
        <ExerciciosManager page={exerciciosPage} onClose={() => setExerciciosPage(null)} />
      )}

      {/* Gerenciador de Cursos */}
      {cursosPage && (
        <CursosManager page={cursosPage} onClose={() => setCursosPage(null)} />
      )}
    </div>
  );
}
